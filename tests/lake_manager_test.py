...
def sample_data() -> pl.DataFrame:
    return pl.DataFrame({"id": [1, 2, 3], "name": ["foo", "bar", "baz"], "value": [1.1, 2.2, 3.3]})


@pytest.fixture
def update_data() -> pl.DataFrame:
    return pl.DataFrame(
        {
            "id": [1, 2, 4],
...
@pytest.fixture
def lake_manager(temp_dir: str) -> LocalLakeManager:
    return LocalLakeManager(base_path=temp_dir)


def test_initialization(temp_dir: str):
...
def test_context_manager(temp_dir: str):
    with LakeManager(base_path=temp_dir) as lake_manager:
        assert isinstance(lake_manager, LakeManager)


def test_create_list_tables(lake_manager: LakeManager, schema: Schema, sample_data: pl.DataFrame):
    lake_manager.create_table(table_name="test_table", table_schema=schema, primary_keys="id")
...
def test_table_operations(lake_manager: LakeManager, schema: Schema, sample_data: pl.DataFrame):
    lake_manager.create_table(table_name="operations_table", table_schema=schema, primary_keys="id")
    ...
    overwrite_data = pl.DataFrame({"id": [5, 6], "name": ["five", "six"], "value": [5.5, 6.6]})
    lake_manager.overwrite_table(table_name="operations_table", df=overwrite_data)
    result = lake_manager.get_data_frame("operations_table")
    assert len(result) == 2
    assert all(id in [5, 6] for id in result["id"])  # noqa: A001


def test_merge_operations(
    lake_manager: LakeManager, schema: Schema, sample_data: pl.DataFrame, update_data: pl.DataFrame
):
    lake_manager.create_table(table_name="merge_table", table_schema=schema, primary_keys="id")
    lake_manager.append_table(table_name="merge_table", df=sample_data)
    lake_manager.merge_table(table_name="merge_table", df=update_data, merge_condition="insert")
    result = lake_manager.get_data_frame("merge_table")
    assert len(result) == 4

    lake_manager.merge_table(table_name="merge_table", df=update_data, merge_condition="update")
    result = lake_manager.get_data_frame("merge_table")
    updated_record = result.filter(pl.col("id") == 1)
    assert updated_record["name"][0] == "updated_foo"

    additional_data = pl.DataFrame({"id": [7, 2], "name": ["seven", "updated_again"], "value": [7.7, 22.2]})
    lake_manager.merge_table(table_name="merge_table", df=additional_data, merge_condition="upsert")
    result = lake_manager.get_data_frame("merge_table")
    assert len(result) == 5
...
def test_optimize_vacuum(lake_manager: LakeManager, schema: Schema, sample_data: pl.DataFrame):
    lake_manager.create_table(table_name="maintenance_table", table_schema=schema, primary_keys="id")
    lake_manager.append_table(table_name="maintenance_table", df=sample_data)
    lake_manager.optimize_table(table_name="maintenance_table", target_size=1024 * 1024, max_concurrent_tasks=2)
...
def test_table_info_and_schema(lake_manager: LakeManager, schema: Schema, sample_data: pl.DataFrame):
    lake_manager.create_table(table_name="info_table", table_schema=schema, primary_keys="id")
...
def test_error_handling(lake_manager: LakeManager, sample_data: pl.DataFrame):
    with pytest.raises(ValueError):
...
def test_delete_table(lake_manager: LakeManager, schema: Schema, sample_data: pl.DataFrame):
    lake_manager.create_table(table_name="delete_table", table_schema=schema, primary_keys="id")
...
def test_merge_to_empty_table(lake_manager: LakeManager, schema: Schema, update_data: pl.DataFrame):
    lake_manager.create_table(table_name="empty_merge_table", table_schema=schema, primary_keys="id")
    empty_result = lake_manager.get_data_frame("empty_merge_table")
    assert len(empty_result) == 0

    lake_manager.merge_table(table_name="empty_merge_table", df=update_data, merge_condition="insert")
    result = lake_manager.get_data_frame("empty_merge_table")
    assert len(result) == 3

    lake_manager.create_table(table_name="empty_upsert_table", table_schema=schema, primary_keys="id")
    lake_manager.merge_table(table_name="empty_upsert_table", df=update_data, merge_condition="upsert")
    result = lake_manager.get_data_frame("empty_upsert_table")
    assert len(result) == 3

    lake_manager.create_table(table_name="empty_update_table", table_schema=schema, primary_keys="id")

    with pytest.raises(ValueError):
        lake_manager.merge_table(table_name="empty_update_table", df=update_data, merge_condition="update")

    with pytest.raises(ValueError):
        lake_manager.merge_table(table_name="empty_update_table", df=update_data, merge_condition="delete")


def test_empty_table_data_access(lake_manager: LakeManager, schema: Schema):
    lake_manager.create_table(table_name="empty_df_table", table_schema=schema, primary_keys="id")
    lake_manager.create_table(table_name="empty_lazy_table", table_schema=schema, primary_keys="id")
    empty_df = lake_manager.get_data_frame("empty_df_table")
    assert len(empty_df) == 0
    assert set(empty_df.columns) == set(schema.keys())
    assert empty_df.schema == dict(schema.items())
    ...
    assert empty_lazy_collected.schema == dict(schema.items())

    sample_data = pl.DataFrame({"id": [1, 2], "name": ["test1", "test2"], "value": [1.1, 2.2]})
